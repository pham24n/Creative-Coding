class ArmLeft extends Arm {
  
  ArmLeft(int n, float x, float y, float w, float h) {
    super(n,x,y,w,h);
    makeBody();
  }
  ...
  void makeBody() {
    for(int i=0; i < numPoints; i++) {
      ...
      s = new Shape(x+i*len,y+i*len,w,h,false); //create shapes that position on the left side of the head
      shapes.add(s);
